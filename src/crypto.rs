...
impl AESCTRHelper {
    ...
    pub fn decrypt(&self, encrypted_data: Vec<u8>) -> Vec<u8> {
        ...
        let mut cipher = ctr::Ctr64BE::<aes::Aes256>::new_from_slices(&self.AESkey, &iv).unwrap();
...
mod test {
    ...
    use crate::{client::parse_action_message, protos::{client::{ReceiveMessagesRequest, SendMessageRequest}, rpc::{OutgoingRPCData, OutgoingRPCMessage}}};
    ...
    fn go_encrypt() {
        ...
        let crypto = AESCTRHelper {
            AESkey: base64::decode("vIYkvvx6mbmr94dmGLkvGMVmrRCEXWsTcpaGMadx7mU=").unwrap(),
            HMACkey: base64::decode("/McGUYgViOW/5m/orqeuecz3Wv+PXkB25fiYlGXys7Y=").unwrap(),
        };
        // let ciphertext = crypto.encrypt(plaintext);
    ...
    fn keygen() {
        let crypto = AESCTRHelper::new();
        println!("aes: {}", base64::encode(&crypto.AESkey));
        println!("hmac: {}", base64::encode(&crypto.HMACkey));
    }

    #[test]
    fn decrypt_web_req() {
        let crypto = AESCTRHelper {
            AESkey: base64::decode("83nxSx6kjEuxJBYC2M7egf0IXW3iuVDmfRGrICs1uqI=").unwrap(),
            HMACkey: base64::decode("rehRi6hvjczvK5KVGftoo0FeZaw/GUakRnvpxPU6WNw=").unwrap(),
        };
